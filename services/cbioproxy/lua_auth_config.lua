access_by_lua '
  ...
  local keycloakClient = os.getenv("KEYCLOAK_CLIENT_FHIRSPARK")
  local keycloak_client_secret = os.getenv("KEYCLOAK_SECRET_FHIRSPARK")

  local opts = {
    redirect_uri = cbioportalUrl .. "/mtb/redirect",
    accept_none_alg = false,
    discovery = keycloakRealm .. "/.well-known/openid-configuration/",
    client_id = keycloakClient,
    client_secret = keycloak_client_secret,
    ssl_verify = "yes",
    prompt = "login",
    logout_path = "/mtb/logout",
    redirect_after_logout_uri = keycloakRealm .. "/protocol/openid-connect/logout?client_id=" .. keycloakClient .. "&redirect_uri=" .. cbioportalUrl .. "%2Fmtb%2Flogin",
